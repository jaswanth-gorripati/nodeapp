...
for (let key in ORGS) {
	if (key.indexOf('org') === 0) {
		forChannel(hfc.getConfigSetting('channelName'),key);
	}
}
function forChannel(channelName,org){
		...
		logger.info(util.format('for Channel >--->  %s', channel));
		clients[org] = client;
...
var getRegisteredUsers1 = function(username,passkey,userOrg, isJson) {
	var member;
	var client = getClientForOrg(userOrg);
			...
			if (user) {
				...
				return getAdminUser(userOrg).then(function(adminUserObj) {
					...
					return caClient.register({
						...
						attrs:[{name:'position', value:"admin",ecert:true}],
						role:"user"
					}, member);
				}).then((secret) => {
					...
					return caClient.enroll({
						enrollmentID: username,
						enrollmentSecret: secret,
						attr_reqs:[{name:'position', optional : true}]
					});
				}, (err) => {
	...
	return hfc.newDefaultKeyValueStore({
		...
		return client.getUserContext(username, true).then((user) => {
			/*if (user && user.isEnrolled()) {
				...
				return caClient.enroll({
					enrollmentID: username,
					enrollmentSecret: passkey,
					attr_reqs:[{name:'id', optional : true},{name:'position', optional : true},{name:'accountType', optional : true},{name:'worksin', optional : true},{name:'studyingIn', optional : true},{name:'isAdmin', optional : true}]
				}).then((message) => {
				if (message && typeof message === 'string' && message.includes(
						'Error:')) {
		...
		logger.error(util.format('Failed to get Enroll user: %s, error: %s', username, err.stack ? err.stack : err));
		return '' + err;
	});
};
var registerUser = function(username,passkey,userOrg,req_attrs,isJson) {
	var users = hfc.getConfigSetting('admins');
		...
		return client.getUserContext(username, true).then((user) => {
			if (user && user.isEnrolled()) {
				...
				return getAdminUser(userOrg).then(function(adminUserObj) {
					...
					logger.debug(username + ' registered successfully with pass code :'+passkey+'');
				}, (err) => {
					logger.debug(username + ' failed to register');
					return '' + err;
					//return 'Failed to register '+username+'. Error: ' + err.stack ? err.stack : err;
...
exports.getRegisteredUsers1 = getRegisteredUsers1;
exports.getOrgAdmin = getOrgAdmin;
exports.forChannel = forChannel;
exports.registerUser = registerUser;
exports.enrollRegisteredUsers = enrollRegisteredUsers;
